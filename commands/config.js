...
const { string, sort } = require('mathjs');
    ...
    if (message.member.hasPermission('ADMINISTRATOR')) {
        if (message.channel.name == 'bot-config') {
            ...
            if (configInput == "student") {
                ...
                configData[serverArrayId].studentId = message.guild.roles.cache.find(role => role.name.toLowerCase() === inputRole.toLowerCase()).id;
                console.table(configData);
                replyMsg += `Your student role has been updated.`;
            } else if (configInput == "teacher") {
                let inputRole = args.slice(1).reduce((a, b) => a + " " + b)
                configData[serverArrayId].teacherId = message.guild.roles.cache.find(role => role.name.toLowerCase() === inputRole.toLowerCase()).id;
                console.table(configData);
                ...
                if (args[1].toLowerCase() == "add") {
                    ...
                    function deleteTimeEntry(dateEntry, weekday) {
        ...
        if (sortedTimeTable.length != 0) {
            ...
            sortedTimeTable.forEach(element => {
                if (weekDay == currentDay) {
                    ...
                    stringToPrint += `${numeral(element.startTime.hours).format("00")}:${numeral(element.startTime.minutes).format("00")} - ${numeral(element.endTime.hours).format("00")}:${numeral(element.endTime.minutes).format("00")}: ${element.subject}\n`;
                }
            });
            stringToPrint += "\n";
        }
