...
import { SchoolSubject } from 'src/entity/subject/schoolSubject';
import { Substitution } from 'src/entity/substitution/substitution';
...
export class TimetableService {
  ...
  */
    ...
    try {
      ...
      const element = await prisma.timeTableElement.create({
        data: {
          ...
          schoolSubjects: {
            connect: {
              schoolSubjectUUID: timeTableElementSubjectUUID,
            },
          },
          schoolRoom: {
            connect: {
  ...
  async getTimeTableElementDetailed(timeTableElementUUID: string, date: string, request): Promise<TimeTableElement> {
    ...
    return new TimeTableElement({
      ...
      timeTableSubstitution: element.timeTableSubstitution.map(substitution => new Substitution({
        ...
        timeTableSubstitutionTeachers: substitution.timeTableSubstitutionTeachers.map(teacher => new User(teacher.users)),
      })),
  ...
  async addHoliday(holiday, request): Promise<Holiday> {
    ...
    if (holidayStartDate > holidayEndDate) throw new BadRequestException();
  ...
  async getHolidayOfSchool(schoolUUID: string): Promise<Holiday> {
    try {
      const holiday = await prisma.schools.findUnique({
        where: {
          schoolUUID,
        },
        include: {
  ...
  async removeHoliday(holidayUUID: string): Promise<Holiday> {
    try {
  ...
  async updateHoliday(
    ...
    const { holidayUUID, holidayName, holidayStartDate, holidayEndDate } = holiday;

    if (holidayStartDate > holidayEndDate) throw new BadRequestException("Enddate cannot be before startdate");
    ...
    try {
      ...
      return new Holiday(updatedHoliday);
    } catch {
      throw new InternalServerErrorException('Database error');
    }
  }
