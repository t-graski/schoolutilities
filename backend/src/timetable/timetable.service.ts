...
export class TimetableService {
  ...
  async addHoliday(holiday, request): Promise<Holiday> {
    ...
    if (holidayStartDate > holidayEndDate) throw new BadRequestException("Start date can't be after end date");
  ...
  async getHolidayOfSchool(schoolUUID: string): Promise<Holiday[]> {
    try {
      ...
      return holidays.holidays.map((holiday) => new Holiday(holiday));
    } catch {
      throw new InternalServerErrorException('Database error');
    }
  }
  ...
  async updateHoliday(
    ...
    if (holidayStartDate > holidayEndDate) throw new BadRequestException('Holiday start date cannot be greater than end date');
